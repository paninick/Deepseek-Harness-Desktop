...
export const en = {
  ...
  visionModelOff: 'Off',
  visionModelBackup: 'Backup vision model',
  visionModelBackupHint: 'Tried automatically when the vision model above fails (timeout, rate limit, provider error).',
  visionModelEndpointHint: 'Routes reuse the selected provider\'s endpoint, protocol, and API key. For a separate vision API, add it as a custom provider first.',
  visionModelMode: 'Vision route policy',
  visionModelModeAuto: 'Auto: primary, then backup',
  visionModelModePrimary: 'Primary only',
  visionModelModeBackup: 'Backup only',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  visionModelOff: '不启用',
  visionModelBackup: '备用识图模型',
  visionModelBackupHint: '上方识图模型失败（超时、限流、服务错误）时自动改用该模型。',
  visionModelEndpointHint: '识图路由复用所选提供方的接口地址、协议和 API 密钥；若识图使用独立接口，请先添加为自定义提供方。',
  visionModelMode: '识图路由策略',
  visionModelModeAuto: '自动：主模型，失败后备用',
  visionModelModePrimary: '仅主模型',
  visionModelModeBackup: '仅备用模型',
