...
describe('connection lifecycle', () => {
  ...
  it('treats describe failure as generation failure and retries', async () => {
    ...
    try {
      ...
      gate.resolve(ok({ version: '0', cwd: '/f', attachedSessions: 0, canOpenPath: true, home: '/fx-home', scratchCwd: '/scratch' }))
      await vi.waitFor(() => { expect(connected).toBe(1) })
    } finally {
      controller.stop()
      warnSpy.mockRestore()
  ...
  it('treats a host.describe business error as generation failure', async () => {
    ...
    api.onDescribe = () => {
      ...
      return Promise.resolve(ok({ version: '0', cwd: '/f', attachedSessions: 0, canOpenPath: true, home: '/fx-home', scratchCwd: '/scratch' }))
    }
    let connected = 0
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => undefined)
    const controller = new ConnectionController(api, { onConnected: () => { connected++ } }, FAST)
  ...
  it('does not open downlinks until host.describe succeeds', async () => {
    ...
    api.onDescribe = () => {
      ...
      return describeCalls === 1
        ...
        : Promise.resolve(ok({ version: '0', cwd: '/f', attachedSessions: 0, canOpenPath: true, home: '/fx-home', scratchCwd: '/scratch' }))
    }
    let connected = 0
    const controller = new ConnectionController(api, { onConnected: () => { connected++ } }, FAST)
    controller.start()
    try {
      ...
      firstDescribe.resolve(ok({ version: '0', cwd: '/f', attachedSessions: 0, canOpenPath: true, home: '/fx-home', scratchCwd: '/scratch' }))
      await vi.waitFor(() => { expect(connected).toBe(1) })
